...
type 'a elt = {v: 'a; from: Config_option.from}

let elt_content elt = elt.v

let elt_from elt = elt.from

let make_elt v from = {v; from}
...
module C = Config_option.Make (struct
  ...
  type 'a config_elt = 'a elt

  type 'a elt = 'a config_elt

  let elt_v = elt_content

  let elt_from = elt_from

  let make_elt = make_elt
...
module Formatting = struct
  ...
  let break_struct =
    ...
    C.choice ~names ~all ~doc ~kind
      (fun conf elt ->
        update conf ~f:(fun f ->
            {f with break_struct= make_elt Poly.(elt.v = `Force) elt.from} )
        )
      (fun conf ->
        let elt = conf.fmt_opts.break_struct in
        if elt.v then make_elt `Force elt.from
        else make_elt `Natural elt.from )
...
let selected_profile_ref = ref {v= `default; from= `Default}
...
let profile =
  ...
  let all =
    [ C.Value.make ~name:"conventional" `conventional
...
let default =
  let elt content = make_elt content `Default in
  let default_elt opt = elt @@ C.default opt in
